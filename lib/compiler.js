...
function newProject(env, ts, _options, _fileNames) {
    ...
    var adapter = factory_1.loadAdapter(ts);
    var cache = null;
    var result = adapter.parseOptions(env, _options, _fileNames);
    ...
    function compile() {
        var result = adapter.compile(options, fileNames, new cache_1.NullCache());
    ...
    function watch(callback) {
        ...
        callback(recompile());
        function recompile() {
            var result = adapter.compile(options, fileNames, cache);
            result.formatter = formatter;
            result.reportDiagnostics();
